...
@MapperScan("com.aynu.campus.mapper")
    ...
    private static void printLog(Environment env) {
        try {
            String protocol = Optional.ofNullable(env.getProperty("server.ssl.key-store"))
                    .map(key -> "https")
                    .orElse("http");
            String serverPort = Optional.ofNullable(env.getProperty("server.port"))
                    .orElse("8080");
            ...
            String hostAddress = InetAddress.getLocalHost()
                    .getHostAddress();
            String appName = env.getProperty("spring.application.name", "User-Service");
            ...
            log.warn("User Service log metadata resolution failed.");
        }
    }
