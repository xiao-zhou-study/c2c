...
public class TurnstileServiceImpl implements ITurnstileService {
    ...
    public TurnstileVerificationResult verifyToken(String token, String remoteip) {
        ...
        if (token == null || token.trim().isEmpty()) {
            ...
            return TurnstileVerificationResult.failure(TurnstileVerificationResult.ErrorCode.INVALID_INPUT_RESPONSE.getCode(),
                    "验证令牌不能为空");
        }
        ...
        try {
            ...
            if (response.getStatusCode() != HttpStatus.OK) {
                ...
                return TurnstileVerificationResult.failure(TurnstileVerificationResult.ErrorCode.SERVICE_UNAVAILABLE.getCode(),
                        "验证服务不可用");
            }
            ...
            if (!success && jsonResponse.has("error-codes")) {
                ...
                if (errorCodes.isArray() && !errorCodes.isEmpty()) {
                    ...
                    log.warn("Turnstile verification failed with error code: {}, message: {}", errorCode, errorMessage);
                }
            }
            ...
            return TurnstileVerificationResult.failure(TurnstileVerificationResult.ErrorCode.UNKNOWN_ERROR.getCode(),
                    "验证失败，请稍后重试");
        }
    }
    ...
    private String getClientIpAddress() {
        try {
            ...
            // 按优先级尝试获取真实 IP
            String[] headers = {"X-Forwarded-For", "X-Real-IP", "X-Client-IP", "CF-Connecting-IP"};
    ...
    private String getErrorMessage(String errorCode) {
        TurnstileVerificationResult.ErrorCode error = TurnstileVerificationResult.ErrorCode.fromCode(errorCode);
