...
public class FilesServiceImpl extends ServiceImpl<FilesMapper, Files> implements IFilesService {
    ...
    public String uploadFile(MultipartFile file, String module) {
        ...
        try (InputStream inputStream = file.getInputStream()) {
            sftpUtil.uploadFile(inputStream, module, fileName);
        } catch (IOException e) {
            log.error("文件上传失败", e);
        ...
        fileEntity.setFilePath(module + "/" + fileName);
        fileEntity.setFileSize(fileSize);
        fileEntity.setFileType(getFileType(extension));
        fileEntity.setMimeType(mimeType);
        fileEntity.setMd5(md5);
        fileEntity.setUrl(uploadProperties.getUrlPrefix() + "/" + module + "/" + fileName);
        ...
        // 8. 返回结果
        return uploadProperties.getUrlPrefix() + "/" + module + "/" + fileName;
    }
