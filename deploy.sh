...
# --- 1. 配置区域 ---
# 项目根目录绝对路径
PROJECT_PATH="/usr/myprojects/backend/wpgx"
# 指定需要编译的服务模块
...
# --- 2. 切换工作目录 ---
echo "[步骤 0/4] 正在进入项目目录: $PROJECT_PATH"
...
mvn clean package -DskipTests -pl $MODULES -am -U
...
docker compose build --no-cache --pull $MODULES
